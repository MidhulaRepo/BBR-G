...
//uint32_t send_pnum = 0; 
//uint32_t recv_pnum = 0;
//uint32_t currentloss_pnum = 0;
//uint32_t previousloss_pnum = 0;
//double losspratio = 0.0;
uint32_t lastSeqNum ;
uint32_t nextAckNumber;
...
TcpSocketBase::TcpSocketBase()
    : TcpSocket()
{
    std::cout<<"..(tcp-socket-base.cc)......TcpSocketBase()....."<<std::endl;
    ...
    std::cout<<".... (TcpSocketBase)...Sending Rate = "<<m_tcb->m_pacingRate<<std::endl; ////getchar();//midhula
...
TcpSocketBase::TcpSocketBase(const TcpSocketBase& sock)
    : TcpSocket(sock),
      ...
      m_delAckCount(0),
      m_totalRetransmissions(0),   //Added by Midhula on April 2, 2024-2.
      ...
      m_ecnCWRSeq(sock.m_ecnCWRSeq)
{
    ...
    //std::cout<<"..(tcp-socket-base.cc)......Copy the rtt estimator if it is set....."<<std::endl;
...
{
    m_rtt = rtt;
    std::cout<<"\n set RTT :"<<m_rtt<<std::endl;////getchar();
}
...
{
    //std::cout<<"..(tcp-socket-base.cc).....Bind()......."<<std::endl;
...
{
    //std::cout<<"..(tcp-socket-base.cc)........SetInitialSSThresh().........."<<std::endl;
    NS_ABORT_MSG_UNLESS(
...
{
    //std::cout<<"..(tcp-socket-base.cc)............GetInitialSSThresh().........."<<std::endl;
...
{
    std::cout<<"..(tcp-socket-base.cc)...............SetInitialCwnd()...."<<std::endl;
    NS_ABORT_MSG_UNLESS(
    ...
    std::cout<<"..(tcp-socket-base.cc)...............//SetInitialCwnd()...."<<m_tcb->m_initialCWnd<<std::endl;
}

uint32_t
TcpSocketBase::GetInitialCwnd() const
{
    std::cout<<"..(tcp-socket-base.cc)...........GetInitialCwnd()....."<<std::endl;
...
{
    //std::cout<<"..(tcp-socket-base.cc)..........Connect()..........."<<std::endl;
    NS_LOG_FUNCTION(this << address);
...
{
    std::cout<<"..(tcp-socket-base.cc)...............Listen()...."<<std::endl; //getchar();
    NS_LOG_FUNCTION(this);
...
TcpSocketBase::Close()
{
    std::cout<<"..(tcp-socket-base.cc).............Close()........"<<std::endl;// getchar();
    NS_LOG_FUNCTION(this);
    /// \internal
    ...
    return DoClose();
   // std::cout<<"..(tcp-socket-base.cc).............//Close()........"<<std::endl;
}
...
{
    //std::cout<<"..(tcp-socket-base.cc)...........ShutdownSend()..."<<std::endl;
    NS_LOG_FUNCTION(this);
...
{
    //std::cout<<"..(tcp-socket-base.cc)...........ShutdownRecv()...."<<std::endl;
    NS_LOG_FUNCTION(this);
    m_shutdownRecv = true;
    return 0;
    //std::cout<<"..(tcp-socket-base.cc)...........//ShutdownRecv()...."<<std::endl;
...
{
    //std::cout<<"..(tcp-socket-base.cc)..........Send()......."<<std::endl;
    ...
    if (m_state == ESTABLISHED || m_state == SYN_SENT || m_state == CLOSE_WAIT)
    {
       ...
       // std::cout<<"Inflight Data = "<< m_tcb->m_bytesInFlight<<std::endl; ////getchar(); //midhula
...
{
    //std::cout<<"..(tcp-socket-base.cc)...........SendTo()......"<<std::endl;
    return Send(p, flags); // SendTo() and Send() are the same
    //std::cout<<"..(tcp-socket-base.cc)...........//SendTo()...SendTo() and Send() are the same..."<<std::endl;
}
...
{
    //std::cout<<"..(tcp-socket-base.cc).......Recv().....Return data to upper-layer application. Data is returned as a packet of size no larger than maxSize ....."<<std::endl;
    NS_LOG_FUNCTION(this);
    ...
    return outPacket;
    //std::cout<<"..(tcp-socket-base.cc).......//Recv().........."<<std::endl;
}
...
{
    //std::cout<<"..(tcp-socket-base.cc).............RecvFrom().........."<<std::endl;
...
{
    //std::cout<<"..(tcp-socket-base.cc)...........GetTxAvailable()....."<<std::endl;
    NS_LOG_FUNCTION(this);
...
{
    //std::cout<<"..(tcp-socket-base.cc).............GetRxAvailable()....Get the max number of bytes an app can read ..."<<std::endl;
    NS_LOG_FUNCTION(this);
    return m_tcb->m_rxBuffer->Available();
    //std::cout<<"..(tcp-socket-base.cc).............//GetRxAvailable()...."<<std::endl;
}
...
{
    //std::cout<<"..(tcp-socket-base.cc).......GetSockName()........"<<std::endl;
    NS_LOG_FUNCTION(this);
    if (m_endPoint != nullptr)
    {
...
{
    //std::cout<<"..(tcp-socket-base.cc).........GetPeerName()......."<<std::endl;
    NS_LOG_FUNCTION(this << address);
...
{
    //std::cout<<"..(tcp-socket-base.cc).........BindToNetDevice()..."<<std::endl;
...
{
    //std::cout<<"..(tcp-socket-base.cc)..........SetupCallback()...."<<std::endl;
    NS_LOG_FUNCTION(this);
...
{
    //std::cout<<"..(tcp-socket-base.cc).......DoConnect().........."<<std::endl;
    NS_LOG_FUNCTION(this);
...
{
    //std::cout<<"..(tcp-socket-base.cc)..............DoClose()......"<<std::endl;
    NS_LOG_FUNCTION(this);
    switch (m_state)
    {
    case SYN_RCVD:
...
{
    //std::cout<<"..(tcp-socket-base.cc).............CloseAndNotify()...."<<std::endl;
    NS_LOG_FUNCTION(this);
...
{
    //std::cout<<"..(tcp-socket-base.cc).................OutOfRange().....if a sequence number range is out side the range that my rx buffer can accept .."<<std::endl;
    ...
    return (tail < m_tcb->m_rxBuffer->NextRxSequence() ||
            ...
            //std::cout<<"..(tcp-socket-base.cc).................OutOfRange().."<<std::endl;
}
...
{
    std::cout<<"..(tcp-socket-base.cc)...ForwardUp()...Function called by the L3 protocol when it received a packet to pass on to the TCP..."<<std::endl;// getchar();
    ...
    //std::cout<<"..(tcp-socket-base.cc)...//ForwardUp()..."<<std::endl;
}
...
{
    std::cout<<"..(tcp-socket-base.cc)...........ForwardUp6()...."<<std::endl; //getchar();
    ...
    DoForwardUp(packet, fromAddress, toAddress);
    //std::cout<<"..(tcp-socket-base.cc).............//ForwardUp6()...."<<std::endl;
}
...
TcpSocketBase::ForwardIcmp(Ipv4Address icmpSource,
                           ...
                           uint32_t icmpInfo)
{
    //std::cout<<"..(tcp-socket-base.cc)............ForwardIcmp()........"<<std::endl;
...
TcpSocketBase::ForwardIcmp6(Ipv6Address icmpSource,
                            ...
                            uint32_t icmpInfo)
{
//std::cout<<"..(tcp-socket-base.cc)............ForwardIcmp6()........"<<std::endl;
    NS_LOG_FUNCTION(this << icmpSource << static_cast<uint32_t>(icmpTtl)
                         << static_cast<uint32_t>(icmpType) << static_cast<uint32_t>(icmpCode)
                         << icmpInfo);
...
{
//std::cout<<"..(tcp-socket-base.cc).......IsValidTcpSegment()..."<<std::endl;
...
{
std::cout<<"..(tcp-socket-base.cc)........DoForwardUp()."<<std::endl; //getchar();
    ...
    if (tcpHeader.GetFlags() & TcpHeader::SYN)
    {
        ...
        m_tcb->m_cWnd = GetInitialCwnd() * GetSegSize();             //cwnd initialized here!!!!!!!!!!!!!!!!!
...
{
//std::cout<<"..(tcp-socket-base.cc)...........ProcessEstablished()......"<<std::endl;
...
{
//std::cout<<"..(tcp-socket-base.cc)............IsTcpOptionEnabled()."<<std::endl;
...
{
//std::cout<<"..(tcp-socket-base.cc)...........ReadOptions()..."<<std::endl;
    NS_LOG_FUNCTION(this << tcpHeader);
...
{
//std::cout<<"..(tcp-socket-base.cc)........EnterCwr()....Sender should reduce the Congestion Window as a response to receiver's ECN Echo notification only once per window..."<<std::endl;
    ...
    //std::cout<<"..(tcp-socket-base.cc)........EnterCwr().."<<std::endl;
}

void
TcpSocketBase::EnterRecovery(uint32_t currentDelivered)
{
//std::cout<<"..(tcp-socket-base.cc).............EnterRecovery().."<<std::endl;
    NS_LOG_FUNCTION(this);
    ...
    m_congestionControl->CongestionStateSet(m_tcb, TcpSocketState::CA_RECOVERY); //commented midhula on 16/09/2023
...
{
std::cout<<"..(tcp-socket-base.cc)..........DupAck()...."<<std::endl;
    NS_LOG_FUNCTION(this);
    ...
    if (m_tcb->m_congState == TcpSocketState::CA_OPEN)
    {
        ...
        m_congestionControl->CongestionStateSet(m_tcb, TcpSocketState::CA_DISORDER); //commented midhula on 16/09/2023
...
{
std::cout<<"..(tcp-socket-base.cc).....ReceivedAck().....Process the newly received ACK...."<<std::endl;//getchar();
    NS_LOG_FUNCTION(this << tcpHeader);
    ...
    // If there is any data piggybacked, store it into m_rxBufferf
    if (packet->GetSize() > 0)
    {
        ReceivedData(packet, tcpHeader);
    }

    SequenceNumber32 xxs= m_highRxAckMark.Get();
    std::cout<<"xxs is "<<xxs<<std::endl;//getchar();

    // RFC 6675, Section 5, point (C), try to send more data. NB: (C) is implemented
    // inside SendPendingData
    SendPendingData(m_connected);
    std::cout<<"..(tcp-socket-base.cc).....ReceivedAck()..."<<std::endl;
}
...
{
//std::cout<<"..(tcp-socket-base.cc)......ProcessAck()...."<<std::endl;
    ...
    else if (ackNumber > oldHeadSequence)
    {
        ...
        std::cout<<"Acked Segments..."<<segsAcked<<std::endl;//midhula
        ...
        else
        {
            ...
            else if (m_tcb->m_congState == TcpSocketState::CA_DISORDER)
            {
                ...
                if (!isDupack)
                {
                    ...
                    m_congestionControl->CongestionStateSet(m_tcb, TcpSocketState::CA_OPEN);            //commented midhula on 16/09/2023
                    m_tcb->m_congState = TcpSocketState::CA_OPEN;  //tesing midhula on 16/09/2023
...
{
//std::cout<<"..(tcp-socket-base.cc)......ProcessListen()......"<<std::endl;
    NS_LOG_FUNCTION(this << tcpHeader);
...
{
//std::cout<<"..(tcp-socket-base.cc)..........ProcessSynSent()..."<<std::endl;
    NS_LOG_FUNCTION(this << tcpHeader);
...
TcpSocketBase::ProcessSynRcvd(Ptr<Packet> packet,
                              ...
                              const Address& /* toAddress */)
{
//std::cout<<"..(tcp-socket-base.cc)........ProcessSynRcvd()..."<<std::endl;
    NS_LOG_FUNCTION(this << tcpHeader);
...
{
//std::cout<<"..(tcp-socket-base.cc).........ProcessWait()..."<<std::endl;
    NS_LOG_FUNCTION(this << tcpHeader);
...
{
//std::cout<<"..(tcp-socket-base.cc).........ProcessClosing()..."<<std::endl;
    NS_LOG_FUNCTION(this << tcpHeader);
...
{
//std::cout<<"..(tcp-socket-base.cc).........ProcessLastAck()..."<<std::endl;
    NS_LOG_FUNCTION(this << tcpHeader);
...
{
//std::cout<<"..(tcp-socket-base.cc)........PeerClose().."<<std::endl;
    NS_LOG_FUNCTION(this << tcpHeader);
...
{
//std::cout<<"..(tcp-socket-base.cc)...........DoPeerClose()."<<std::endl;
...
{
//std::cout<<"..(tcp-socket-base.cc).....Destroy().."<<std::endl;
    NS_LOG_FUNCTION(this);
    m_endPoint = nullptr;
    if (m_tcp)
    {
...
{
//std::cout<<"..(tcp-socket-base.cc).....Destroy6().."<<std::endl;
    NS_LOG_FUNCTION(this);
    m_endPoint6 = nullptr;
    if (m_tcp)
    {
...
{
//std::cout<<"..(tcp-socket-base.cc).....SendEmptyPacket()..."<<std::endl;
...
{
//std::cout<<"..(tcp-socket-base.cc).....SendRST()..."<<std::endl;
    NS_LOG_FUNCTION(this);
...
{
//std::cout<<"..(tcp-socket-base.cc).....DeallocateEndPoint()..."<<std::endl;
...
{
//std::cout<<"..(tcp-socket-base.cc).....SetupEndpoint()..."<<std::endl;
    NS_LOG_FUNCTION(this);
...
{
//std::cout<<"..(tcp-socket-base.cc).....SetupEndpoint6()..."<<std::endl;
    NS_LOG_FUNCTION(this);
...
TcpSocketBase::CompleteFork(Ptr<Packet> p [[maybe_unused]],
                            ...
                            const Address& toAddress)
{
    //std::cout<<"..(tcp-socket-base.cc)....CompleteFork()..."<<std::endl;
...
{ 
    std::cout<<"..(tcp-socket-base.cc).....ConnectionSucceeded()..."<<std::endl; //getchar();
// Wrapper to protected function NotifyConnectionSucceeded() so that it can
    ...
    std::cout<<"..(tcp-socket-base.cc)....//.ConnectionSucceeded()..."<<std::endl;
}

void
TcpSocketBase::AddSocketTags(const Ptr<Packet>& p) const
{
    //std::cout<<"..(tcp-socket-base.cc).....AddSocketTags().."<<std::endl;
    /*
...
{
    //std::cout<<"..(tcp-socket-base.cc).....SendDataPacket().....Extract at most maxSize bytes from the TxBuffer at sequence seq, add the TCP header, and send to TcpL4Protocol..."<<std::endl;
    
    m_tcb->m_send_pnum++;

    NS_LOG_FUNCTION(this << seq << maxSize << withAck);

    bool isStartOfTransmission = BytesInFlight() == 0U;  //checks if this is the start of a transmission by seeing if there are no bytes currently in flight.
    TcpTxItem* outItem = m_txBuffer->CopyFromSequence(maxSize, seq); //creates a copy of the data from the transmission buffer starting at `seq` with a maximum size of `maxSize`.

    m_rateOps->SkbSent(outItem, isStartOfTransmission);

    bool isRetransmission = outItem->IsRetrans(); //checks if the outgoing item is a retransmission.
   // uint32_t isRetransmissionCount;
    // if (isRetransmission == 1)
    // {
    //     isRetransmissionCount++; 
    //     std::cout<<"RETR STATUS  "<<isRetransmissionCount<<std::endl;getchar(); //midhula

    // }
    //std::cout<<"RETR STATUS  "<<isRetransmission<<std::endl;getchar(); //midhula
    Ptr<Packet> p = outItem->GetPacketCopy();
    uint32_t sz = p->GetSize(); // Size of packet
    uint8_t flags = withAck ? TcpHeader::ACK : 0;  //sets the ACK flag if `withAck` is true.
    uint32_t remainingData = m_txBuffer->SizeFromSequence(seq + SequenceNumber32(sz)); //Determines how much data remains to be sent from the transmission buffer after this packet.
    ...
    if (m_endPoint)
    {
        ...
        NS_LOG_DEBUG("Send segment of size "
                     ...
                     << m_endPoint->GetPeerAddress() << "..(tcp-socket-base.cc).. Header " << header);
    }
    else
    {
        m_tcp->SendPacket(p,
        ...
        NS_LOG_DEBUG("Send segment of size "
                     << sz << " with remaining data " << remainingData << " via TcpL4Protocol to "
                     << m_endPoint6->GetPeerAddress() << "..(tcp-socket-base.cc).. Header " << header);
    }
    ...
    SequenceNumber32 xxm = m_tcb->m_highTxMark.Get();
    std::cout<<"xxm is "<<xxm<<std::endl;//getchar();

    //// Calculate the sequence number of the last packet sent
    uint32_t lastSeqNum = seq + SequenceNumber32(sz) - SequenceNumber32(1);
    //std::cout << "Sequence number of the last packet sent: " << lastSeqNum << std::endl; //getchar();

    return sz;

    //std::cout<<"..(tcp-socket-base.cc)...//..SendDataPacket(..."<<std::endl;
}

void
TcpSocketBase::UpdateRttHistory(const SequenceNumber32& seq, uint32_t sz, bool isRetransmission)
{
    //std::cout<<"..(tcp-socket-base.cc)......UpdateRttHistory()..."<<std::endl;
    NS_LOG_FUNCTION(this);
...
{
    //std::cout<<"..(tcp-socket-base.cc).....SendPendingData()...."<<std::endl;
        ...
        else
        {
            ...
            if (!m_noDelay && UnAckDataCount() > 0 && availableData < m_tcb->m_segmentSize)
            {
                NS_LOG_DEBUG("Invoking Nagle's algorithm for seq "
                             ...
                             << "..(tcp-socket-base.cc).. Wait to send.");
                break;
            }
...
{
    //std::cout<<"..(tcp-socket-base.cc).......UnAckDataCount() "<<std::endl;
    return m_tcb->m_highTxMark - m_txBuffer->HeadSequence();
   // std::cout<<"//......UnAckDataCount() "<<std::endl;
}

uint32_t
TcpSocketBase::BytesInFlight() const
{
   // std::cout<<"..(tcp-socket-base.cc)......BytesInFlight()..."<<std::endl;
    ...
    //std::cout<<"Buffered inflight"<<bytesInFlight<<std::endl;////getchar();      //midhula
...
{
   // std::cout<<"..(tcp-socket-base.cc)......Window()..."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc).....AvailableWindow()..."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)......AdvertisedWindowSize()..."<<std::endl;
...
TcpSocketBase::ReceivedData(Ptr<Packet> p, const TcpHeader& tcpHeader)
{
   // std::cout<<"..(tcp-socket-base.cc).....ReceivedData()..."<<std::endl;
    ...
    // Put into Rx buffer
    SequenceNumber32 expectedSeq = m_tcb->m_rxBuffer->NextRxSequence();  //Retrieves the next expected sequence number from the receive buffer (`m_tcb->m_rxBuffer->NextRxSequence()`) to compare with the received packet's sequence number.
    ...
    //Determines whether to send an acknowledgment (ACK) packet immediately or delay it.
    ...
    // Assuming the packet has been added to the receive buffer and you're ready to acknowledge it:
...
{
    std::cout<<"..(tcp-socket-base.cc).....NewAck()..."<<std::endl; //getchar();
    ...
    //std::cout<<"..(tcp-socket-base.cc)....//.NewAck()..."<<std::endl;
}
...
{
    //std::cout<<"..(tcp-socket-base.cc)....ReTxTimeout()..Retransmit timeout."<<std::endl;
    NS_LOG_FUNCTION(this);
   ...
   // std::cout<<"Retransmission Count = "<<m_dataRetries<<std::endl; //getchar(); //Midhula Added
    ...
    m_congestionControl->CongestionStateSet(m_tcb, TcpSocketState::CA_LOSS); //commented midhula on 16/09/2023
    ...
    NS_ASSERT_MSG(BytesInFlight() <= m_tcb->m_segmentSize,
                  "In flight (" << BytesInFlight() << ") there is more than one segment ("
                                << m_tcb->m_segmentSize << ")");
    //std::cout<<"..(tcp-socket-base.cc)..//..ReTxTimeout()..."<<std::endl;
}

void
TcpSocketBase::DelAckTimeout()
{
    //std::cout<<"..(tcp-socket-base.cc).....DelAckTimeout()..."<<std::endl;
    m_delAckCount = 0;
    m_congestionControl->CwndEvent(m_tcb, TcpSocketState::CA_EVENT_DELAYED_ACK);
    if (m_tcb->m_ecnState == TcpSocketState::ECN_CE_RCVD ||
        m_tcb->m_ecnState == TcpSocketState::ECN_SENDING_ECE)
...
{
    //std::cout<<"..(tcp-socket-base.cc).....LastAckTimeout()...."<<std::endl;
    NS_LOG_FUNCTION(this);
...
{
    //std::cout<<"..(tcp-socket-base.cc)........PersistTimeout()........"<<std::endl;
...
{
    std::cout<<"..(tcp-socket-base.cc)........DoRetransmit()..."<<std::endl; //getchar();

    std::ofstream outputFile("Retransmission Count.txt", std::ios_base::app);
    ...
    //NextSeg() function retrieves the sequence number of the segment to retransmit.
    ...
    else  //it means a segment was found for retransmission, and maxSizeToSend is set to the size of this segment.
    {
        // NextSeg() may constrain the segment size when res is true
        maxSizeToSend = static_cast<uint32_t>(seqHigh - seq);

//check the outputFile opened correctly
        if (outputFile.is_open())
        {
        outputFile<<m_totalRetransmissions<<std::endl;
        
        // Increment the retransmission counter
        m_totalRetransmissions++;                           //Added by Midhula on April 2, 2024-3
        std::cout << "Retransmitting packet Count: " << m_totalRetransmissions<<std::endl; //getchar();//Added by Midhula on April 2, 2024 -4
        outputFile.close();
        }
        else
        {
            std::cerr<<"Unable to open thefile "<<std::endl;
        }
    }
    ...
    // Print the retransmitted packet details
    std::cout << "Retransmitting packet with sequence number: " << seq 
              << " and size: " << maxSizeToSend << " bytes." << std::endl; //getchar();  //added by midhula
    

    NS_ASSERT(sz > 0);
    std::cout<<"..(tcp-socket-base.cc)......//..DoRetransmit()..."<<std::endl;
}

void
TcpSocketBase::CancelAllTimers()
{
    //std::cout<<"..(tcp-socket-base.cc)........CancelAllTimers()..."<<std::endl;
    m_retxEvent.Cancel();
    m_persistEvent.Cancel();
    m_delAckEvent.Cancel();
    m_lastAckEvent.Cancel();
...
{
    //std::cout<<"..(tcp-socket-base.cc)........TimeWait()..."<<std::endl;
...
{
    //std::cout<<"..(tcp-socket-base.cc)........SetSndBufSize()..."<<std::endl;
...
{
//std::cout<<"..(tcp-socket-base.cc)........GetSndBufSize() .."<<std::endl;
...
{
    //std::cout<<"..(tcp-socket-base.cc)........SetRcvBufSize() .."<<std::endl;
    NS_LOG_FUNCTION(this << size);
...
{
    //std::cout<<"..(tcp-socket-base.cc)........GetRcvBufSize().."<<std::endl;
...
{
    //std::cout<<"..(tcp-socket-base.cc)........SetSegSize().."<<std::endl;
    NS_LOG_FUNCTION(this << size);
...
{
    //std::cout<<"..(tcp-socket-base.cc)........GetSegSize().."<<std::endl;
...
{
    //std::cout<<"..(tcp-socket-base.cc)........SetConnTimeout().."<<std::endl;
...
{
    //std::cout<<"..(tcp-socket-base.cc)........GetConnTimeout().."<<std::endl;
    return m_cnTimeout;
   //std::cout<<"..(tcp-socket-base.cc).......//.GetConnTimeout().."<<std::endl;
}
...
{
    //std::cout<<"..(tcp-socket-base.cc)........SetSynRetries().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)........GetSynRetries() .."<<std::endl;
    return m_synRetries;
   // std::cout<<"..(tcp-socket-base.cc).......//.GetSynRetries() .."<<std::endl;
}
...
{
   // std::cout<<"..(tcp-socket-base.cc)........SetDataRetries() .."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)........GetDataRetries()  .."<<std::endl;
    NS_LOG_FUNCTION(this);
...
{
   // std::cout<<"..(tcp-socket-base.cc)........SetDelAckTimeout()  .."<<std::endl;
    NS_LOG_FUNCTION(this << timeout);
...
{
   // std::cout<<"..(tcp-socket-base.cc).......GetDelAckTimeout()  .."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc).......SetDelAckMaxCount()  .."<<std::endl;
    NS_LOG_FUNCTION(this << count);
...
{
   // std::cout<<"..(tcp-socket-base.cc).......GetDelAckMaxCount() .."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc).......SetTcpNoDelay() .."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc).......GetTcpNoDelay().."<<std::endl;
    return m_noDelay;
  //  std::cout<<"..(tcp-socket-base.cc).....//..GetTcpNoDelay().."<<std::endl;
}
...
{
  //  std::cout<<"..(tcp-socket-base.cc).......SetPersistTimeout().."<<std::endl;
    NS_LOG_FUNCTION(this << timeout);
...
{
  //  std::cout<<"..(tcp-socket-base.cc).......GetPersistTimeout().."<<std::endl;
...
TcpSocketBase::SetAllowBroadcast(bool allowBroadcast)
{
  ...
  //   std::cout<<"..(tcp-socket-base.cc).......SetAllowBroadcast().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc).......GetAllowBroadcast().."<<std::endl;
    return false;
  //  std::cout<<"..(tcp-socket-base.cc).....//..GetAllowBroadcast().."<<std::endl;
}
...
{
   // std::cout<<"..(tcp-socket-base.cc).......AddOptions().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc).......ProcessOptionWScale().."<<std::endl;
...
{
  //  std::cout<<"..(tcp-socket-base.cc).......CalculateWScale().."<<std::endl;
    NS_LOG_FUNCTION(this);
    ...
    {
        maxSpace = maxSpace >> 1;
        //std::cout<<"Maximum Buffer Space...."<<maxSpace<<std::endl;//getchar();//midhula not printing
        ++scale;
    }
...
{
  //  std::cout<<"..(tcp-socket-base.cc).......AddOptionWScale().."<<std::endl;
    NS_LOG_FUNCTION(this << header);
...
{
   // std::cout<<"..(tcp-socket-base.cc).......ProcessOptionSack().."<<std::endl;
    NS_LOG_FUNCTION(this << option);
...
{
   // std::cout<<"..(tcp-socket-base.cc).......ProcessOptionSackPermitted().."<<std::endl;
    NS_LOG_FUNCTION(this << option);
...
{
   // std::cout<<"..(tcp-socket-base.cc)......AddOptionSackPermitted().."<<std::endl;
    NS_LOG_FUNCTION(this << header);
    NS_ASSERT(header.GetFlags() & TcpHeader::SYN);
...
{
  //  std::cout<<"..(tcp-socket-base.cc)......AddOptionSack().."<<std::endl;
    NS_LOG_FUNCTION(this << header);
...
{
   // std::cout<<"..(tcp-socket-base.cc)......ProcessOptionTimestamp().."<<std::endl;
    NS_LOG_FUNCTION(this << option);
...
{
  //  std::cout<<"..(tcp-socket-base.cc)......AddOptionTimestamp().."<<std::endl;
    NS_LOG_FUNCTION(this << header);
...
{
    std::cout<<"..(tcp-socket-base.cc)......UpdateWindowSize().."<<std::endl;
    NS_LOG_FUNCTION(this << header);
...
{
 //   std::cout<<"..(tcp-socket-base.cc)......SetMinRto().."<<std::endl;
...
{
  //  std::cout<<"..(tcp-socket-base.cc)......GetMinRto().."<<std::endl;
    return m_minRto;
  //  std::cout<<"..(tcp-socket-base.cc).....//.GetMinRto().."<<std::endl;
}
...
{
   // std::cout<<"..(tcp-socket-base.cc)......SetClockGranularity().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)......GetClockGranularity().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)......GetTxBuffer().."<<std::endl;
    return m_txBuffer;
  //  std::cout<<"..(tcp-socket-base.cc).....//.GetTxBuffer().."<<std::endl;
}
...
{
  //  std::cout<<"..(tcp-socket-base.cc)......GetRxBuffer().."<<std::endl;
...
{
  //  std::cout<<"..(tcp-socket-base.cc)......SetRetxThresh().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)......UpdatePacingRateTrace().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)......UpdateCwnd().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)......UpdateCwndInfl().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)......UpdateSsThresh().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)......UpdateCongState().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)......UpdateEcnState().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)......UpdateNextTxSequence().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)......UpdateHighTxMark().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)......UpdateBytesInFlight().."<<std::endl;
...
{
    std::cout<<"..(tcp-socket-base.cc)......UpdateRtt()......m_lastRttTrace ...........found.."<<std::endl;
    m_lastRttTrace(oldValue, newValue);         //..........................m_lastRttTrace ...........found.
    std::cout<<"..(tcp-socket-base.cc)....//..UpdateRtt().."<<std::endl;
}

void
TcpSocketBase::SetCongestionControlAlgorithm(Ptr<TcpCongestionOps> algo)
{
   // std::cout<<"..(tcp-socket-base.cc)......SetCongestionControlAlgorithm().."<<std::endl;
...
{
  //  std::cout<<"..(tcp-socket-base.cc)......SetRecoveryAlgorithm().."<<std::endl;
...
TcpSocketBase::Fork()
{
   // std::cout<<"..(tcp-socket-base.cc)......Fork().."<<std::endl;
...
{
   // std::cout<<"..(tcp-socket-base.cc)......SafeSubtraction().."<<std::endl;
    if (a > b)
    {
        return a - b;
    }
...
{
    //std::cout<<"..(tcp-socket-base.cc)......NotifyPacingPerformed().."<<std::endl;
    NS_LOG_FUNCTION(this);
...
{
    //std::cout<<"..(tcp-socket-base.cc)......IsPacingEnabled().."<<std::endl;
    if (!m_tcb->m_pacing)
    {
        return false;
    }
...
{
    std::cout<<"..(tcp-socket-base.cc)......UpdatePacingRate().."<<std::endl;
    std::ofstream outputFile10("UpdatePacingRate.dat", std::ios_base::app);
    NS_LOG_FUNCTION(this << m_tcb);
    std::cout<<"Pacing Data rate is = "<< m_tcb->m_pacingRate<<std::endl; //getchar(); //midhula

    //Check if the file opened correctly
    if (outputFile10.is_open())
    {
        outputFile10<<Simulator::Now ().GetSeconds () << " " <<m_tcb->m_pacingRate<<std::endl;
        outputFile10.close();
    }
    else
    {
    ...
    if (m_tcb->m_cWnd < m_tcb->m_ssThresh / 2) // It was /2
    {
    ...
    //std::cout<<"Last RTT is = "<< lastRtt<<std::endl; //getchar(); //midhula not printing
    ...
    else
    {
        ...
        m_tcb->m_pacingRate = m_tcb->m_maxPacingRate;
        //std::cout<<"Pacing Data rate is = "<< m_tcb->m_maxPacingRate<<std::endl; //getchar(); //midhula it is not printing
    }
    std::cout<<"..(tcp-socket-base.cc).....//.UpdatePacingRate().."<<std::endl; 
}
...
{
    //std::cout<<"..(tcp-socket-base.cc).....SetPacingStatus().."<<std::endl;
...
{
    //std::cout<<"..(tcp-socket-base.cc).....SetPaceInitialWindow().."<<std::endl;
...
{
    //std::cout<<"..(tcp-socket-base.cc)....SetUseEcn().."<<std::endl;
...
{
    //std::cout<<"..(tcp-socket-base.cc)....GetRWnd().."<<std::endl;
    return m_rWnd.Get();
    //std::cout<<"..(tcp-socket-base.cc)..//..GetRWnd().."<<std::endl;
}
...
{
    //std::cout<<"..(tcp-socket-base.cc)....GetHighRxAck().."<<std::endl;
