...
TraceThroughput (Ptr<FlowMonitor> monitor)
{
  ...
  if(itr!=stats.end()){
    std::ofstream thr (dir + "/1_throughput.dat", std::ios::out | std::ios::app);
  ...
  itr = stats.find(3);
  if(itr!=stats.end()){
    std::ofstream thr (dir + "/2_throughput.dat", std::ios::out | std::ios::app);
    thr <<  curTime.GetSeconds () << " " << 8 * (itr->second.rxBytes - prev2) / (1000 * 1000 * (curTime.GetSeconds () - prevTime.GetSeconds ())) << std::endl;
...
void CheckQueueSize (Ptr<QueueDisc> qd)
{
  ...
  std::cout<<"Qsize="<<qsize<<std::endl; //getchar();       //midhula
  q.close ();
}
...
void TraceCwnd (uint32_t nodeId, uint32_t socketId)
{
  ...
  Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream (dir + "/"+ id + "_cwnd.dat");
...
int main (int argc, char *argv [])
{
  ...
  struct tm * timeinfo;
  char buffer [80];
  //May I change the buffer size?  Shallow buffer is 80, deep buffer is 200 : Testing. Midhula
  time (&rawtime);
  ...
  std::cout<<tcpTypeId<<std::endl;

  NodeContainer sender, receiver, sender2, receiver2;
  ...
  bottleneckLink.SetDeviceAttribute  ("DataRate", StringValue ("30Mbps"));                 //Revision 1.1
  bottleneckLink.SetChannelAttribute ("Delay", StringValue ("10ms"));

  PointToPointHelper edgeLink; //edgeLink1;
  //PointToPointHelper edgeLink1; //Fair 1
  edgeLink.SetDeviceAttribute  ("DataRate", StringValue ("1000Mbps"));
  edgeLink.SetChannelAttribute ("Delay", StringValue ("5ms"));
  //edgeLink1.SetDeviceAttribute  ("DataRate", StringValue ("1000Mbps"));  //Fair 2
  //edgeLink1.SetChannelAttribute ("Delay", StringValue ("25ms"));      //Fair 3
  ...
  //NetDeviceContainer sender2Edge = edgeLink1.Install (sender2.Get (0), routers.Get (0));  //Fair 4
  ...
  sourceApps2.Start (Seconds (8.1));
  // Hook trace source after application starts
  Simulator::Schedule (Seconds (8.1) + MilliSeconds (1), &TraceCwnd, 0, 0);
  ...
  // Check for dropped packets using Flow Monitor step 2.2
  ...
  Simulator::Run ();
  //Flow monitor Step 2.2
  monitor->SerializeToXmlFile("MonitorFile.xml",true,true);
  //Step 3... Run the file "./waf --run scratch/tcp-bbr-example_Mohit.cc"
  //then check for .xml file in the home directory of ns3.(Let's say x.xml)
  //copy the file from "src/flow-monitor/examples/flowmon-parse-results.py" to home directory
  //Step 4  .. Parse this file using "python flowmon-parse-results.py x.xml". We will get statistics of every flow in the network.
