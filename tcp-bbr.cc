    ...
    static TypeId tid =
        TypeId("ns3::TcpBbr")
            ...
            .AddAttribute("ProbeRttDuration",
                          ...
                          TimeValue(MilliSeconds(200)), //200
...
TcpBbr::SetPacingRate(Ptr<TcpSocketState> tcb, double gain)
{
    ...
    {
        outputFile<<Simulator::Now ().GetSeconds () << " " <<gain<< " " <<gain_C<<" "<<tcb->m_rttFactor<<std::endl;
        outputFile.close();
    }
    else
    {
    ...
    {
        outputFile100<<Simulator::Now ().GetSeconds () << " " <<tcb->m_pacingRate<<" "<<gain<<" "<< C<<" " <<std::endl;
        outputFile100.close();
    }
    else
    {
...
{
    NS_LOG_FUNCTION(this << tcb << gain);
    std::ofstream outputFile1("Gain_Inflight.txt", std::ios_base::app);
...
TcpBbr::AdvanceCyclePhase()
{
   ...
   // std::cout<<"M-PACINGGAIN="<<m_pacingGain<<std::endl;getchar(); //midhula
}
...
TcpBbr::CheckFullPipe(const TcpRateOps::TcpRateSample& rs)
{
    ...
    if (m_fullBandwidthCount >= 3)  //it was 3
    {
...
TcpBbr::EnterDrain()
{
    ...
    m_pacingGain = 1.0 / m_highGain; //m1
    m_cWndGain = m_highGain; //m3
}
...
TcpBbr::EnterProbeBW()
{
    ...
    m_pacingGain = 1; //m2
    m_cWndGain = 2; //m4
    m_cycleIndex = GAIN_CYCLE_LENGTH - 1 - (int)m_uv->GetValue(0, 6); //m5
    AdvanceCyclePhase();
}
...
TcpBbr::SaveCwnd(Ptr<const TcpSocketState> tcb)
{
    ...
    {
        m_priorCwnd = std::max(m_priorCwnd, tcb->m_cWnd.Get());  //original
...
TcpBbr::ModulateCwndForProbeRTT(Ptr<TcpSocketState> tcb)
{
    ...
    if (m_state == BbrMode_t::BBR_PROBE_RTT)
    {
       tcb->m_cWnd = std::min(tcb->m_cWnd.Get(), m_minPipeCwnd);   //Original BBR ProbeRTT 
...
TcpBbr::SetCwnd(Ptr<TcpSocketState> tcb, const TcpRateOps::TcpRateSample& rs)
{
    ...
    if (!rs.m_ackedSacked)  // If there haven't been any acknowledgments
    {
        goto done;
    }
    ...
    {
        // if (tcb->m_currRtt > 2 * tcb->m_minRtt)   //for BBR-A 1
        // {
        //     tcb->m_cWnd = tcb->m_cWnd / 2;
        //     m_pacingGain = 0.75;
        //     //gain_C = 0.75;
        // }
        if (ModulateCwndForRecovery(tcb, rs))       //older 
        {
         goto done;
        }
    }
...
TcpBbr::CongestionStateSet(Ptr<TcpSocketState> tcb, const TcpSocketState::TcpCongState_t newState)
{
    ...
    else if (newState == TcpSocketState::CA_RECOVERY)
    {
        ...
        // if (tcb->m_currRtt > 2 * tcb->m_minRtt)  //BBR-A 2
        // {
        //    tcb->m_cWnd = tcb->m_cWnd / 2;
        //    m_pacingGain = 0.75;
        //     //gain_C = 0.75;
        ...
        // else
        tcb->m_cWnd =
            tcb->m_bytesInFlight.Get() + std::max(tcb->m_lastAckedSackedBytes, tcb->m_segmentSize);  //older
